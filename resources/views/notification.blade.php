@extends('layouts.main')

@section('container')
    <main class="flex-1 h-screen p-10 mt-14 relative">
        <div class="absolute bg-[#96C3ED] w-[110px] h-[110px] rounded-full ms-36 top-1/4"></div>
        <div class="absolute bg-[#35AFE1] w-[85px] h-[85px] rounded-full right-2/3 bottom-2/4 translate-y-56"></div>
        <div class="absolute bg-main w-[140px] h-[140px] right-3 rounded-full top-2/3 -translate-x-48"></div>
        <div class="relative">
            <h2 class="flex px-2 rounded-sm font-inter border-l-4 border-main font-bold items-center text-main text-lg z-50">
                <div class="bg-white h-10 w-10 me-5 rounded-full flex justify-center items-center">
                    <svg class="fill-main w-6 h-6" viewBox="0 0 30 30" fill="none" xmlns="http://www.w3.org/2000/svg"
                        xmlns:xlink="http://www.w3.org/1999/xlink">
                        <rect width="30" height="30" fill="url(#pattern0)" />
                        <defs>
                            <pattern id="pattern0" patternContentUnits="objectBoundingBox" width="1" height="1">
                                <use xlink:href="#image0_40_982" transform="scale(0.0111111)" />
                            </pattern>
                            <image id="image0_40_982" width="90" height="90"
                                xlink:href="data:image/png;base64," />
                        </defs>
                    </svg>
                </div>
                Notifikasi
            </h2>

            <table class=" flex items-center justify-center mt-5">
                <tbody class="text-sm font-normal text-gray-700 text-center rounded-xl ">
                    <tr class=" backdrop-blur font-medium outline outline-black outline-1 rounded-xl ">
                        <td class="ps-7"><svg class="fill-main w-9 h-9" viewBox="0 0 30 30" fill="none"
                                xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
                                <rect width="30" height="30" fill="url(#pattern0)" />
                                <defs>
                                    <pattern id="pattern0" patternContentUnits="objectBoundingBox" width="1"
                                        height="1">
                                        <use xlink:href="#image0_40_982" transform="scale(0.0111111)" />
                                    </pattern>
                                    <image id="image0_40_982" width="90" height="90"
                                        xlink:href="data:image/png;base64," />
                                </defs>
                            </svg></td>
                        <td class="px-10 py-6 ">Abas</td>
                        <td class="px-10 py-6">SWH-0271-23</td>
                        <td class="px-10 py-6">Interface LACP-BUNGAH (>90%)</td>
                        <td class="px-10 py-6 flex">
                            <div class="bg-green-500 rounded-full w-4 h-4 text-center me-2"></div> Update
                        </td>
                    </tr>
                    <td height="20"></td>
                    </tr>
                    <tr class=" backdrop-blur font-medium outline outline-black outline-1 rounded-xl ">
                        <td class="ps-7"><svg class="fill-main w-9 h-9" viewBox="0 0 30 30" fill="none"
                                xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
                                <rect width="30" height="30" fill="url(#pattern0)" />
                                <defs>
                                    <pattern id="pattern0" patternContentUnits="objectBoundingBox" width="1"
                                        height="1">
                                        <use xlink:href="#image0_40_982" transform="scale(0.0111111)" />
                                    </pattern>
                                    <image id="image0_40_982" width="90" height="90"
                                        xlink:href="data:image/png;base64," />
                                </defs>
                            </svg></td>
                        <td class="px-10 py-6 ">Aliah</td>
                        <td class="px-10 py-6">SWH-0789-23</td>
                        <td class="px-10 py-6">CPU Temperature thresshold >60</td>
                        <td class="px-10 py-6 flex">
                            <div class="bg-orange-500 rounded-full w-4 h-4 text-center me-2"></div> Update
                        </td>
                    </tr>
                    <td height="20"></td>
                    </tr>
                    <tr class=" backdrop-blur font-medium outline outline-black outline-1 rounded-xl ">
                        <td class="ps-7"><svg class="fill-main w-9 h-9" viewBox="0 0 30 30" fill="none"
                                xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
                                <rect width="30" height="30" fill="url(#pattern0)" />
                                <defs>
                                    <pattern id="pattern0" patternContentUnits="objectBoundingBox" width="1"
                                        height="1">
                                        <use xlink:href="#image0_40_982" transform="scale(0.0111111)" />
                                    </pattern>
                                    <image id="image0_40_982" width="90" height="90"
                                        xlink:href="data:image/png;base64," />
                                </defs>
                            </svg></td>
                        <td class="px-10 py-6 ">Roni</td>
                        <td class="px-10 py-6">SWH-0987-22</td>
                        <td class="px-10 py-6">Unavailable by ICMP </td>
                        <td class="px-10 py-6 flex">
                            <div class="bg-red-700 rounded-full w-4 h-4 text-center me-2"></div> Update
                        </td>
                    </tr>
                    <tr>
                        <td height="20"></td>
                    </tr>
                    <tr class=" backdrop-blur font-medium outline outline-black outline-1 rounded-xl ">
                        <td class="ps-7"><svg class="fill-main w-9 h-9" viewBox="0 0 30 30" fill="none"
                                xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
                                <rect width="30" height="30" fill="url(#pattern0)" />
                                <defs>
                                    <pattern id="pattern0" patternContentUnits="objectBoundingBox" width="1"
                                        height="1">
                                        <use xlink:href="#image0_40_982" transform="scale(0.0111111)" />
                                    </pattern>
                                    <image id="image0_40_982" width="90" height="90"
                                        xlink:href="data:image/png;base64," />
                                </defs>
                            </svg></td>
                        <td class="px-10 py-6 ">Sindy</td>
                        <td class="px-10 py-6">SWH-0987-22</td>
                        <td class="px-10 py-6">Unavailable by ICMP</td>
                        <td class="px-10 py-6 flex">
                            <div class="bg-red-700 rounded-full w-4 h-4 text-center me-2"></div> Update
                        </td>
                    </tr>
                </tbody>
            </table>
    </main>
@endsection
