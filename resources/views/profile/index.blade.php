    ...
    <main class="flex-1 min-h-screen p-10 mt-14">
        ...
        <section class="flex justify-center items-center">
            <div class="relative">
                ...
                <div
                    class="flex relative justify-center px-10 py-14 w-[430px] min-h-max backdrop-blur m-16 border border-[#515151] rounded-xl">
                    ...
                    <div class="flex flex-col gap-3 w-full">
                        <div class="flex justify-center">
                            <a href="/profile/{{ auth()->user()->id }}/edit"
                                class="bg-main border-2 border-main hover:bg-transparent py-1 px-4 rounded-full text-sm font-medium text-white duration-200 hover:text-main">Edit
                                Profile</a>
                        </div>
                        <form action="#" class="flex flex-col gap-3 w-full mt-7">
                            <div class="flex items-center justify-between">
                                ...
                                <input disabled type="text" name="nama" id="nama"
                                    value="{{ auth()->user()->name }}" class="h-8 border-none rounded-md w-[270px]" />
                            </div>
                            <div class="flex items-center justify-between">
                                <label for="username" class="text-black/50">Email</label>
                                <input disabled type="text" name="username" id="username"
                                    value="{{ auth()->user()->email }}" class="h-8 border-none rounded-md w-[270px]" />
                            </div>
                            {{-- <div class="flex items-center justify-between">
                            <label for="password" class="text-black/50">Password</label>
                            <input disabled type="password" name="password" id="password" value="{{ auth()->user()-> }}"
